...
| 维度 | Efficient Design | dynamicPPO | SelfAttention Encoder-Decoder |
|---|---|---|---|
| 基本定位 | 传统离散设计优化 | Efficient 的 RL 升级版 | dyppo 之上的结构化生成升级版 |
| 变量集合 | 固定 | 固定 | 可选，可激活/关闭 |
| attribute levels | 固定离散 | 固定离散 | 可连续生成，不限于离散 levels |
| 动作本质 | 选出一组离散题 | 从离散 feasible combo pool 选题 | 生成题目结构、mask、value、stop |
...
### 2.2 dyppo -> SelfAttention Encoder-Decoder

升级内容：
- 不再局限于从离散 combo 池中选题
- 可以决定变量激活与否
- 可以为变量生成连续值
- block 可以按生成机制构造，而不是只从固定离散题库抽取
...
```text
Efficient 解决的是固定离散设计的优化问题；dyppo 解决的是在同一离散设计空间上动态调整 combo 的问题；SelfAttention encoder-decoder 则进一步把变量是否出现、变量取什么值、block 如何生成都纳入可学习过程。三者并非简单的强弱关系，而是对应不同的建模层级、灵活性需求与计算成本。
```
