...
DEFAULT_DESIGN_SAVE_NAME = '中途_xjh_限制1'
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Debug flow for selfattention using 中途_xjh_限制1 config.')
...
def run(args: argparse.Namespace) -> None:
    ...
    if verbose:
        print(f"[selfattention/debug] run_dir={RUN_DIR}", flush=True)
    ...
    if verbose:
        print(
            f"[selfattention/debug] loaded design: save_name={design_save_name} preview_tasks={len(preview_tasks)}",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"[selfattention/debug] rows ready: generated={len(rows)} loaded={len(rows_for_train)} attr_dims={len(row_meta.get('attr_dim_names', []) or [])}",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"[selfattention/debug] candidate pool ready: size={len(candidate_pool)} overlap_with_expert={overlap_count}",
            flush=True,
        )
    ...
    for i, row in enumerate(rows, start=1):
        if verbose:
            print(
                ...
                f"n_tasks={len(row.get('tasks') or [])}",
                flush=True,
            )
